 ...
 */
struct GRADIENT *generate_palette (const struct CHANNEL *points, struct GRADIENT *gradient) {
...
struct CHANNEL *add_point (struct CHANNEL *in, double x, double y) {
	struct POINT *new;
	new = realloc(in->p, (in->length + 1) * sizeof(*(in->p)));
	if (!new)
...
/* allocate storage for, and store, points as
 * specified, for each channel, all at once
 */
void add_syncpoint (struct CHANNEL *points, double x, char *line, off_t *off) {
	uint8_t c;
	double y;
	for (c = 0; c < CHANNELS; c++) {
...
void printparsed (struct CHANNEL *points) {
	uint8_t c;
	size_t i;
	for (c = 0; c < CHANNELS; c++)
...
int main (int argc, char **argv) {
	uint8_t c;
	size_t i;
	struct CHANNEL points[CHANNELS];
